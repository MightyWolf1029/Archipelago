...
from .OoTR.Location import DisableType
from .OoTR.Location import Location as OoTRLocation
from .OoTR.LocationList import location_table
...
class OOTLocation(APLocation, OoTRLocation):
    """
    A class to represent a location in OoT. Inherits from AP's base ``Location`` class as well as
    OoTR's ``Location``.
    """
    ...
    def __init__(self, player, name='', code=None, address1=None, address2=None,
        ...
        APLocation.__init__(self, player, name, code, parent)
...
def LocationFactory(locations, player: int):
    """
    Modified implementation of OoTR Location's ``LocationFactory``. It accepts the argument ``player``
    instead of ``world`` and utilizes AP's implementation of ``OOTLocation``.
    """
    ret = []
    singleton = False
...
def build_location_name_groups() -> dict:
    """
    Helper function to build the location name groups dictionary.
    """
